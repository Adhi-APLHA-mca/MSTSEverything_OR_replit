...
using Orts.Parsers.Msts.OrProtection;
using System;
...
namespace Orts.Parsers.Msts
{
    ...
    public class STFReader : IDisposable
    {
        ...
        {
            string item = ReadItem(false);
            // Layer 1 reverse mapping: translate obfuscated codes back to real token names.
            // ReadString() calls ReadItem(true) directly (bypasses this method) so string
            // VALUES are never accidentally unmapped — only structural token names are.
            return isProtectedFile ? OrTokenMap.Unmap(item) : item;
        }
...
#line default
        ...
        /// <summary>True when this reader was opened from an OR-protected route file.</summary>
        private bool isProtectedFile;
